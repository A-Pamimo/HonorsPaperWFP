...
for (ct in unique(d$country)) {
  ...
  for (y in num_outs) {
    ...
    ggsave(outp, p, width = 7, height = 4)
    log_msg("Saved figure:", outp)
  }
}
...
if ("LCS_Cat" %in% names(d)) {
  p2 <- d %>%
    mutate(LCS_Cat = fct_explicit_na(as_factor_safe(LCS_Cat), na_level = "Unknown")) %>%
    count(Remote, LCS_Cat) %>% group_by(Remote) %>% mutate(prop = n/sum(n)) %>% ungroup() %>%
    ggplot(aes(x = Remote, y = prop, fill = LCS_Cat)) +
    geom_col() + theme_minimal() + labs(title="LCS severity share by modality", y="Proportion", x="")
  outp2 <- here::here("output","figures","LCS_severity_stack.pdf")
  ggsave(outp2, p2, width = 6, height = 4)
  log_msg("Saved figure:", outp2)
}
