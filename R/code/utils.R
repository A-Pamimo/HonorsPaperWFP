...
suppressPackageStartupMessages({
  # light deps; your analysis scripts load the heavy ones
...
# ---- 1) safe factor / label handling ------------------------------------------
as_factor_safe <- function(x) {
  # if labelled, convert via haven; else coerce to factor safely
...
write_tex <- function(tex_obj, path) {
  # tex_obj can be a character string (LaTeX) or a gt/modelsummary object
...
write_xlsx_sheets <- function(sheets, path) {
  # sheets: named list of data.frames / tibbles
  dir.create(dirname(path), recursive = TRUE, showWarnings = FALSE)
...
# ---- 4) cluster picker (PSU > enumerator > robust) ----------------------------
cluster_picker <- function(df) {
  # highest priority: PSU-type clusters; then enumerator; else NULL (robust HC)
  psu_candidates  <- c("ADMIN3Name","ADMIN2Name","psu","cluster","S_Geo_Admin3","S_Geo_Admin2")
  enum_candidates <- c("EnuName","EnuName_Diay","EnuName_Diry","EnuName_Di~y",
                       "enumerator","enum_id")

  pick_first <- function(cands) {
    nm <- names(df)
    for (c in cands) {
      # allow ~ as wildcard (e.g., EnuName_Di~y)
      c_regex <- gsub("~", ".*", c)
      hit <- nm[stringr::str_detect(nm, stringr::fixed(c)) |
                 stringr::str_detect(nm, stringr::regex(c_regex, ignore_case = TRUE))]
      if (length(hit)) {
        # ensure >1 non-missing unique values
  ...
  if (!is.na(psu)) {
    log_msg("cluster_picker: clustering on PSU: ", psu)
    return(psu)
  } else if (!is.na(enum)) {
    log_msg("cluster_picker: clustering on enumerator: ", enum)
    return(enum)
  } else {
    log_msg("cluster_picker: No cluster variable found; using robust HC1/HC2")
    return(NULL)
  }
}

# ---- 5) balance table (Welch t-test) ------------------------------------------
balance_fun <- function(df, vars, group = "Remote") {
  ...
  dplyr::bind_rows(lapply(vars, function(v) {
    ...
    x <- df[[v]]
    g <- df[[group]]
    # coerce logical/factor to numeric share if needed
    if (is.logical(x)) x <- as.numeric(x)
    if (is.factor(x))  x <- as.numeric(x) # for proportion-like coded factors this is fine
    # groups
    ...
    x0 <- xv[gv == levs[1]]
    x1 <- xv[gv == levs[2]]
    # Welch two-sample t-test (handles unequal variances)
    ...
    tibble::tibble(
      ...
      diff       = (mean(x1, na.rm = TRUE) - mean(x0, na.rm = TRUE)), # group1 - group0
      p_value    = pval,
...
# ---- 6) extract Remote betas for plotting/table --------------------------------
...
# For modelsummary-LaTeX numeric wrapping surprises:
