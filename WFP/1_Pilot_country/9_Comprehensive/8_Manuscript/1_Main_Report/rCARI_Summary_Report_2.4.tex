...
\SetWatermarkText{DRAFT - DO NOT SHARE\\	\\INTERNAL USE ONLY}
...
Using a survey experiment methodology, this study's objective is to compare and validate the rCARI methodology of computing and reporting on food security with the standard CARI methodology. The study is being piloted across six countries in Africa, Latin America, the Middle-East, and Asia to access how different context may affect the indicator. The study has been implemented in three pilot countries so far – Myanmar (1,093 interviews in total), Ecuador (1,416 completed interviews), and Central African Republic (1,245 completed interviews), where data collection and analysis has been completed as of the end of August 2023. Data collection in the remaining three countries is expected to start soon. The sample frame for the study is defined uniquely for each study country to capture different population types. In Myanmar where because of accessibility issues the study was conducted among internally displaced persons (IDPs)) in 2 ADMIN1 areas involving 25 Camps. In Ecuador and Central African Republic (CAR), the sampling frames were the general population from all the randomly selected ADMIN5 areas. Sampling of respondents in Myanmar was done by randomly selecting the households from the list of IDPs received from the camp management while in Ecuador and CAR, respondents were selected through a random walk in each selected ADMIN5 area (village). The random walk was considered as a viable option in the absence of census data for each of the ADMIN5 areas. Sampled households in each pilot country were randomly assigned to one of two groups, i.e., Face-to-Face (F2F ) group, which received the standard CARI modules in F2F interviews or the Remote group, which received the rCARI interview by phone. 

Analysis of the data is premised on comparing different versions of CARI using remote and F2F modules. Three versions of CARI were then calculated and compared with the standard CARI. These include: 1) CARI using FES from remote surveys, 2.) CARI using the income variable from F2F surveys, and 3) CARI using income sources and income change variable from remote surveys (this is the version of CARI that was implemented at the height of the Covid pandemic).  Aside from the main comparison of CARI vs. rCARI, the study also compared different versions of the standard FES module (long vs. short versions) to consider implementation in remote surveys. There is also a comparison of LCS-FS vs LCS-EN modules to assess their comparability.

The results, though preliminary, show a number of interesting findings. For example, the results rCARI calculated using the income \& income change indicator (rCARI-Inc) are not statistically different from the standard CARI calculated using the food expenditure share indicator (CARI-FES) in Myanmar \& Ecuador; however, it significantly underestimates food insecurity by 21 percentage points compared to the CARI-FES in CAR. In addition, replacing the F2F with the income source and income change indicator in the FES surveys significantly underestimated food insecurity by between 6\% to 22\% points. Furthermore, FCS, rCSI, FES and income sources indicators collected over phone tend to give similar or slightly different results than in F2F data collection, with differences in FCS (0 to 8 points), rCSI (-1 to 4 points) FES (5\% - 8\% points), and income sources (0 to 8\% points in category 4) (1\% to 11\% points in category 3). Finally, administering the LCS module in its current form over the phone is problematic and may need further refinement and enumerator training. However, in terms of testing the different versions of modules (LCS-FS vs. LCS-EN), the results show that there are no statistically significant differences.
 
Though in the early stages as yet in terms of the validity or otherwise of rCARI, a number of significant deductions can be made based on these results.
\begin{itemize}[label=--, left=1.5em] 
	\item [1.] Conventional rCARI (income) largely under reports FI (food insecurity).  
	\item [2.] Almost all food security indicators are largely comparable between F2F and Remote modalities (where different, it is minimal). 
	\item [3.] Food insecurity estimates using the FES indicator as an absolute value is always higher compared to the income change indicator regardless of modality of data collection and thus replacing FES with the income indicator can significantly lead to under-reporting acute food insecurity numbers.
\end{itemize} 

In the next steps, a further in-depth analysis of the data will be conducted to better understand the differences between the results of the modalities, including potential drivers. Given the potential of the income source ad income change indicator, efforts will be made to harmonize and standardize the indicator to be used in food security assessments and reporting.
...
The overall goal of this study was to assess the validity of the rCARI module(s) against conventional CARI modules for measuring food security within a given population. The study compares indicators collected through phone surveys with conventional face-to-face surveys, considering the latter as the standard methodology. It is important to clarify that the objective of this study is not to report food security at the national level or aim for external validity across the entire population of each pilot country. Instead, the primary goal is to assess the comparability of two methodologies—standard Consolidated Approach for Reporting Food Insecurity (CARI) and remote CARI (rCARI)—when deployed within the same population.

By focusing on the same population within each pilot country, the study seeks to understand how the two methodologies perform under similar conditions and contexts. This approach allows for a direct comparison of the effectiveness and accuracy of the two methodologies in assessing food security indicators.
...
Nevertheless, there are several statistical weighting techniques that can adjust the results of phone surveys based on baseline variables like demographics characteristics of the most recent F2F surveys (Nagpal et al. 2021; Glazerman et al. 2020; Valliant et al. 2013; Elkasabi, 2015). This allows the final results of such exercises to be aggregated and extrapolated to reflect some level of representativeness. However, this current study may not be affected by these confounding issues since the main idea is not to report the results at the national level but rather to evaluate the comparability of remote household interviews in collecting and reporting food security. For this reason, the current study report covering the 3 pilot countries does not use any weights. Nonetheless, the study design adopted in Central African Republic (CAR), could allow for testing of sample skewness due to mobile phone ownership and coverage (see CAR sample design in Annex). CAR has one of the lowest mobile phone penetration rates globally (approximately 70\% of the population do not own mobile phones). In the next section, the study design and the detailed sampling procedure adopted in each country as well as the study implementation is outlined.
...
\subsection{Survey Instrument(s)}
In this section, a brief description of the survey instruments is provided detailing the differences between F2F and remote surveys. Taking advantage of the unique experimental design of the study, we tested different versions of the standardized modules defined according to the length of the questionnaire, short and long modules. Expenditure and food consumption score were the two modules that had variations for this purpose.

\subsubsection{F2F Questionnaire}
	
The Face-to-Face (F2F) questionnaire used in this assessment comprises standard food security assessment modules that have been consistently employed by the World Food Programme (WFP). These modules cover various aspects of household food security, providing comprehensive insights into household demographics, food consumption patterns, coping strategies, livelihoods, expenditures, essential needs, income sources, assets, and the Food Insecurity Experience Scale (FIES) modules.

The F2F questionnaire is available in two versions: short and long. The version deployed depends on the length of items related to expenditure and food consumption modules. The long questionnaire includes all standard expenditure and food consumption items typically queried in food security assessments. It encompasses a total of 154 questions, covering detailed inquiries into both food and non-food items related to household expenditures.

In contrast, the short version of the questionnaire is a condensed version, aimed at reducing respondent burden and interview duration. It consolidates some expenditure items into groups and aggregates expenditure data for these groups rather than listing individual items. As a result, the short version comprises fewer questions, totaling 108 in all.

		
\subsubsection{Remote Questionnaire}

The remote questionnaire is an adapted version of the standard Computer-Assisted Telephone Interviewing (CATI) modules, which have undergone rigorous testing and validation for remote data collection. It includes the same modules as the Face-to-Face (F2F) questionnaire, with some exceptions such as the exclusion of expenditure, asset, and health modules. Typically it is difficult to collect expenditure data in phone interviews due to time constraints and potential respondent fatigue. However, in this experiment, a variant of the standard F2F expenditure module was piloted remotely. This involved condensing and aggregating the number of expenditure items under food and non-food categories, aiming to explore the feasibility of collecting expenditure data remotely.

Another notable difference between the remote and F2F questionnaires lies in the Food Consumption (FC) module. While the remote questionnaire only asks about main food groups, the F2F version includes a comprehensive list of different food items and sub-groups. This difference allows for a more detailed assessment of food consumption patterns in the F2F interviews compared to the remote approach. Overall, the remote questionnaire maintain consistency with the F2F questionnaire while addressing practical considerations and limitations associated with remote data collection. 

Similar to the F2F questionnaire, the remote questionnaire also offers two variants: the long and short versions, differentiated by the number of expenditure questions included. The long version of the remote questionnaire consists of 23 questions related to both food and non-food expenditure items. On the other hand, the short version of the remote questionnaire comprises only 5 questions for both food and non-food expenditure items. The idea is to test which version of the two may be close to the standard expenditure module and allow for collecting expenditure data remote for food security assessments.


\subsection{Survey protocols}

Before, during, and after the survey, several protocols were established to ensure the successful collection of high-quality data. These protocols were designed to address the challenges of low response rate and to promote transparency, trust, and participation among respondents.

Firstly, collaboration with local authorities and influential individuals in each community (ADMIN5s) was initiated, facilitated by WFP Country Offices (COs) and survey contractors. This engagement aimed to sensitize potential respondents to the survey, fostering transparency and increasing trust. By involving key stakeholders at the local level, the survey team was able to garner support and cooperation from the community, laying the foundation for successful data collection.

Secondly, a stand-alone phase household listing (sampling) was implemented, allowing enumerators to inform households of the upcoming interviews during the listing phase. This proactive approach ensured that households were aware of the survey and had the opportunity to choose a convenient time for the interviews. By providing advance notice and allowing households to schedule interviews at their convenience, the risk of ambushing respondents or calling them at inconvenient times was minimized, enhancing the likelihood of participation and cooperation. Overall, these pre-survey protocols were essential for establishing rapport with the community, building trust among respondents, and ensuring that data collection proceeded smoothly and effectively. By proactively addressing potential challenges and engaging stakeholders at the local level, the survey team was able to maximize response rates and achieve high-quality data outcomes. 

In addition to the pre-survey protocols aimed at community engagement and scheduling, a comprehensive calling protocol was developed in collaboration with the hunger monitoring team of the World Food Programme (WFP). Leveraging their expertise in remote data collection, this protocol was designed to ensure consistency, accuracy, and professionalism during phone interviews. The calling protocol outlined guidelines and procedures for enumerators to follow when conducting interviews remotely. These guidelines covered various aspects of the interview process, including:
\begin{itemize}[label=--, left=2.75em]
	\item[1.]\textbf{Introduction:} Enumerators were instructed on how to introduce themselves and the purpose of the survey to respondents in a clear and professional manner, fostering trust and cooperation from the outset.

	\item[2.]\textbf{Consent:} Procedures for obtaining informed consent from respondents were outlined, emphasizing the importance of respecting respondent privacy and autonomy throughout the interview process.

	\item[3.]\textbf{Questioning Techniques:} Enumerators were trained on effective questioning techniques to elicit accurate and comprehensive responses from respondents while maintaining a respectful and non-biased approach.

	\item[4.]\textbf{Data Recording:} Guidelines for accurately recording respondent answers and maintaining data integrity were provided to ensure consistency and reliability in the collected data.
...
In this section, we present the main results of the study by comparing food security indicators using different versions of the Consolidated Approach for Reporting Food Insecurity (CARI) calculated either using remote or face-to-face (F2F) modules with the standard CARI. The objective is to assess the comparability of these methodologies through group mean tests, specifically the student t-test.

The food security indicators analyzed include the following:
\begin{itemize}[label=--, left=2.75em]
	\item[1.]\textbf{Food Consumption Score (FCS)}
	\item[2.]\textbf{Reduced Coping Strategies Index (rCSI)}
	\item[3.]\textbf{Food Expenditure Score (FES)}
	\item[4.]\textbf{Livelihood Coping Strategies}
	\item[5.]\textbf{Income Sources and Income Change}
\end{itemize}
For each indicator, we conducted group mean tests to compare the results obtained from the different versions of CARI. This analysis allows us to evaluate the statistical differences between the remote and F2F modules compared to the standard CARI. The results of the group mean tests provide insights into the comparability of food security indicators derived from remote with F2F data obtained using the standard CARI methodology. Statistical significance levels of 95\%, 99\% and 99.9\% were determined to assess the extent of differences between the methodologies.

Overall, this analysis aims to elucidate the extent to which food security assessments conducted remotely align with those conducted through traditional F2F interviews. These findings contribute to our understanding of the reliability and validity of remote data collection methods in assessing food security in hard to reach in emergency situations.

\subsection{Sample Completion}

Tables 1 \& 2 show the total sample and number of household interviews completed by country and modality in the 3 pilot countries of the study respectively.

\input{1_table/rCARI_Overall_Completion_F2F} 
\label{tab:rCARI_Overall_Completion_F2F}

\input{1_table/rCARI_Overall_Completion_R} 
\label{tab:rCARI_Overall_Completion_R}

As expected, the response rate in the F2F sample (Table 2) was higher than the remote (Table 1)in all three countries. This was primarily due to the poor phone connection/network and not picking up the  phone calls, and less than 1\% of refusals by respondents. Nevertheless, this did not influence the sample balance for almost all the collected background information at the household level for Ecuador and Myanmar. 
...
Next, a comparison of CARI computed using FES is compared with income and income change variable modules asked to the same household/respondent in the same modality. This step allowed a comparison of these two indicators using the same respondents in the same modality. Results for the paired-sample test for the F2F modality are presented in Table 6, while that for the Remote modality is presented in Table 7. The main conclusion from Table 6 is that CARI computed using the income and income change variables significantly under reports the numbers of food insecure across all pilot countries (6\% in Myanmar, 23\% in Ecuador and 22\% in CAR). 
	...
	\begin{table}[htbp]
		...
		\begin{adjustbox}{max width=\textwidth}
			\begin{threeparttable}
				\caption{CARI: FES vs Income Indicators - Paired Responses (F2F Surveys)}
				\scriptsize
...
The results of comparing FES and income indicator using same respondent households in the remote modality show similar trend of respondents significantly under-reporting their food insecurity using the income source and income change indicator by between 12 and 24 percentage points compared to the FES indicator (12\% in Myanmar, 19\% in Ecuador and 24\% in CAR, see table 7). 
	...
	\begin{table}[htbp]
		...
		\begin{adjustbox}{max width=\textwidth}
			\begin{threeparttable}
				\caption{rCARI: FES vs. Income - Paired Reponses (Remote Surveys)}
				\scriptsize
...
This section of the analysis tested whether expenditures could be measured through administering fewer questions. Four different versions of the expenditure module (2 versions for the F2F modality \& the 2 versions for the remote modality) differentiated by the number of questions it entails were carved and tested.
	...
	\begin{itemize}[left=1.5em]
		\item\textbf{F2F Modality}
			...
			\item[1.] Standard expenditure module that contains all standard household expenditure items (a total of 154 questions comprising of food and non-food expenditure).
		...
		\item\textbf{Remote Modality} 
			...
			\item[2.] Extremely short expenditure module (3 collapsed food and 2 non-food expenditure questions respectively).
		\end{itemize}
...
From the preliminary analysis of the available data, the following initially conclusions are made: 
	...
	\begin{itemize}[left=1.5em]
		...
		\item LCS collected by phone tended to produce a higher estimation of food insecurity than those collected through F2F. This difference was mainly driven by LCS crisis + emergency coping (18-28 percentage points).
...
% ------- 
\section{Annexes}
\subsection{Balance tables based on observable characteristics}
\begin{table}[htbp]
		\centering
		\begin{adjustbox}{max width=\textwidth}
			\begin{threeparttable}
				\caption{Myanmar Sample Balance Table (completed survey)}
				\scriptsize
				\input{1_table/Myanmar_Modality_Demo_Balance}\label{tab:Myanmar_Modality_Demo_Balance}
				\begin{tablenotes}[para,flushleft]
					\item Note: Robust standard errors in parentheses; * p$<$0.05, ** p$<$0.01, *** p$<$0.001. 
				\end{tablenotes}
			\end{threeparttable}
...
\begin{table}[htbp]
		...
		\begin{adjustbox}{max width=\textwidth}
			\begin{threeparttable}
				\caption{Ecuador Sample Balance Table (completed survey)}
				\scriptsize
...
\end{table}
	...
	\begin{table}[htbp]
		...
		\begin{adjustbox}{max width=\textwidth}
			\begin{threeparttable}
				\caption{CAR Sample Balance Table (completed surveys)}
				\scriptsize
				\input{1_table/CAR_Modality_Demo_Balance} 
				\label{tab:CAR_Modality_Demo_Balance}
				\begin{tablenotes}[para,flushleft]
					\item Note: Robust standard errors in parentheses; * p$<$0.05, ** p$<$0.01, *** p$<$0.001. \\
...
\begin{table}[htbp]
		...
		\begin{adjustbox}{max width=\textwidth}
			\begin{threeparttable}
				\caption{CAR Sample Balance Table (Phone owner subsample)}
				\scriptsize
...
\subsubsection{Calling protocol}
\includepdf[pages={1-2}]{3_doc/Calling_protocol.pdf}
