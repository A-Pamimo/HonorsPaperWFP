...
*******************************************************************************/
	...
	log using "${d_log}/ed01_setup_roster_`cdate'.smcl", replace
	di `cdate'
	
* load dataset
* ------------
	use "${d_temp}/Ecuador_F2F_Roster_Correct.dta", clear 
	compress
	
	* create unique ID for each individual 
	makeid HH_Index ___index, gen(Ind_Index) project(Ecuador)
...
*******************************************************************************/
	...
	save "${d_temp}/Ecuador_Roster_Prep.dta", replace
	...
	** Only keep household level aggregate variables
	...
	save "${d_temp}/Ecuador_Roster_Household_Prep.dta", replace

********************************************************************************
*						PART 5: Merge to Household F2F dataset
*******************************************************************************/

	use "${d_temp}/Ecuador_F2F_Household_Correct.dta", clear
	
	mmerge HH_Index using "${d_temp}/Ecuador_Roster_Household_Prep.dta", ///
		   type(1:1) uname(R_)
	...
	compress
	save "${d_temp}/Ecuador_F2F_Household_Prep.dta", replace
