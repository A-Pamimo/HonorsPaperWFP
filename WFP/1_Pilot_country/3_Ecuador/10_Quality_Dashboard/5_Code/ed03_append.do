...
********************************************************************************
	...
	** CREATES:		${d_dta}/Ecuador_Full_Household_Analysis_`cdate'.dta
					${d_dta}/Ecuador_Full_Household_Analysis.sav
...
*******************************************************************************/
	...
	log using "${d_log}/ed03_append_`cdate'.smcl", replace
	di `cdate'	
...
*******************************************************************************/
	...
	* load dataset
	* ------------
	use "${d_temp}/Ecuador_F2F_Household_Analysis.dta", clear
	
	* append Remote Answers to F2F Answers
	append using "${d_temp}/Ecuador_RM_Household_Analysis.dta"
	compress
	drop S_*
...
foreach var of global other_specify {
	...
	export excel HHID ${other_specify} 							///
		   using "${note}/Ecuador_other_specify_raw.xlsx", 		///
	...
	* save dataset
	compress
	iecodebook export using "${d_out}/Ecuador_Full_Codebook.xlsx", replace
	
	save 	 "${d_dta}/Ecuador_Full_Household_Analysis.dta", replace
	save 	 "${dta}/Ecuador_Full_Household_Analysis.dta", replace
	save 	 "${all_dta}/Ecuador_Full_Household_Analysis.dta", replace
	
	// not date specific for tableau import consistency
	*savespss "${d_dta}/Ecuador_Full_Household_Analysis.sav", replace
