...
********************************************************************************
	...
	** NOTES:		In the current set up, Nicole focused on household and head 
					of household level characteristics besides other respondent 
					level checks for data quality. (Outcome variables and analysis
					both at household level mostly)
...
*******************************************************************************/
	...
	log using "${d_log}/md01_setup_roster_`cdate'.smcl", replace
	di `cdate'
	
* load dataset
* ------------
	use "${d_temp}/Myanmar_F2F_Roster_Correct.dta", clear 
	compress
...
*	bysort HH_Index: egen Num_Household_check = max(getP)
	...
	** Health Var Summary for Lorenzo
	dtable i.MDDIHHNoSchoolNb_6M i.PHealthBirth i.MDDIHHHealthPreg 				///
		  ...
		  export(${tabs}/Myanmar_rCARI_Roster_Health_Var_Summary.docx, replace)
...
*******************************************************************************/
	...
	save "${d_temp}/Myanmar_Roster_Prep.dta", replace
	...
	** Only keep household level aggregate variables
	...
	save "${d_temp}/Myanmar_Roster_Household_Prep.dta", replace

********************************************************************************
*						PART 5: Merge to Household F2F dataset
*******************************************************************************/

	use "${d_temp}/Myanmar_F2F_Household_Correct.dta", clear
	
	mmerge HH_Index using "${d_temp}/Myanmar_Roster_Household_Prep.dta", ///
		   type(1:1) uname(R_)
	...
	compress
	save "${d_temp}/Myanmar_F2F_Household_Prep.dta", replace
