...
********************************************************************************
	...
	** REQUIRES:	${hfc_temp}/BF_F2F_Roster_Correct.dta
					
	** CREATES:		${hfc_temp}/BF_Roster_Prep.dta
					${hfc_temp}/BF_Roster_Household_Prep.dta
...
* load dataset
* ------------
	use "${hfc_temp}/BF_F2F_Roster_Correct.dta", clear 
	compress
	
	* create unique ID for each individual 
	makeid HH_Index ___index, gen(Ind_Index) project(BF)
...
*******************************************************************************/
	...
	save "${hfc_temp}/BF_Roster_Prep.dta", replace
	...
	** Only keep household level aggregate variables
	...
	save "${hfc_temp}/BF_Roster_Household_Prep.dta", replace

********************************************************************************
*						PART 5: Merge to Household F2F dataset
*******************************************************************************/

	use "${hfc_temp}/BF_F2F_Household_Correct.dta", clear
	
	mmerge HH_Index using "${hfc_temp}/BF_Roster_Household_Prep.dta", ///
		   type(1:1) uname(R_)
	...
	compress
	save "${hfc_temp}/BF_F2F_Household_Prep.dta", replace
