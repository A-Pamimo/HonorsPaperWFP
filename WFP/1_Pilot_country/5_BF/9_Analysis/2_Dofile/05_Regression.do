...
*******************************************************************************/
	...
	eststo clear
	estpost tabulate CARI_Inc_Cat CARI_FES_Cat if Modality == 1
	...
	tabplot CARI_FES_Cat CARI_Inc_Cat if Modality == 1, showval
	
		eststo clear
	estpost tabulate CARI_Inc_Cat CARI_FES_Cat if Modality == 2
	esttab using "${out_tab}/BF_RM_Cat_Cross_Tabulation.tex", ///
    cell("b(fmt(0)) rowpct(fmt(2))") unstack noobs ///
	collabels("Count" "Percent") nonumbers replace mtitles(FES)
	
	tabplot CARI_FES_Cat CARI_Inc_Cat  if Modality == 2, showval
	
	eststo  clear
	estpost tabulate rCARI_Inc_Good CARI_FES_Good if Modality == 1
	esttab using "${out_tab}/BF_FS_Cross_Tabulation.tex", 					///
		   ...
		   collabels("Count" "Percent") nonumbers replace 
		   	...
		   	eststo  clear
	estpost tabulate rCARI_Inc_Good CARI_FES_Good if Modality == 2
	esttab using "${out_tab}/BF_RM_Cross_Tabulation.tex", 					///
		   cell("b(fmt(0)) rowpct(fmt(2))") unstack noobs modelwidth(15) 		///
		   collabels("Count" "Percent") nonumbers replace 
