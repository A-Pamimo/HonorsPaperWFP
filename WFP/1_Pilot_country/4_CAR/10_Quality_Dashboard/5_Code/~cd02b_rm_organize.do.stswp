...
*******************************************************************************/
	...
	log using "${d_log}/cd02b_rm_organize_`cdate'.smcl", replace
	di `cdate'
	
	* load dataset
	use "${d_temp}/CAR_RM_Household_Correct.dta", clear 
...
** Economic vulnerability: Income
	...
	replace HHIncFirst_Accept = 1 if HHIncFirst_oth == "Pasteur"
			
	gen		HHIncFirst_Border = inlist(HHIncFirst_SRi, 3, 4, 6, 12)
	replace HHIncFirst_Border = 1 if HHIncFirst_oth == "Coturier" | ///
			HHIncFirst_oth == "Cultuvateure" | ///
...
*******************************************************************************/
	...
	compress	
*	iecodebook export using "${d_out}/CAR_RM_Codebook.xlsx", replace
	
	save "${d_temp}/CAR_RM_Household_Analysis.dta", replace	
