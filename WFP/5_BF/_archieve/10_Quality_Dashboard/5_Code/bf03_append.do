...
*******************************************************************************/
	...
	* load dataset
	* ------------
	use "${d_temp}/BKF_F2F_Household_Analysis.dta", clear
	
	* append Remote Answers to F2F Answers
	append using "${d_temp}/BKF_RM_Household_Analysis.dta"
	compress
	drop S_*
...
foreach var of global other_specify {
	...
	export excel HHID Modality ${other_specify} 					///
		   using "${note}/BKF_other_specify_raw_`cdate'.xlsx", 		///
...
*	iecodebook export using "${d_out}/CAR_Full_Codebook.xlsx", replace
	...
	save 	 "${d_dta}/BKF_Full_Household_Analysis.dta", replace
	save 	 "${dta}/BKF_Full_Household_Analysis.dta", replace
	save 	 "${all_dta}/BKF_Full_Household_Analysis.dta", replace
	
	// not date specific for tableau import consistency
	savespss "${d_dta}/BKF_Full_Household_Analysis.sav", replace
