...
*******************************************************************************/
	...
	* Outcome Balance by Survey Modality
	iebaltab ${out_oth}, grpvar(Modality) nonote onerow							///
             ...
             rowvarlabels savetex("${tex_tab}/Ecuador_Outcome_Modality_Balance.tex")

	iebaltab ${out_cat}, grpvar(Modality) nonote onerow							///
			 nototal replace format(%12.2fc) grplabels(1 F2F @ 2 Remote)		///
             rowvarlabels savetex("${tex_tab}/Ecuador_CARI_Modality_Balance.tex")
			 
	iebaltab ${out_fes} if Modality == 1, grpvar(Form)   						///
			 nonote nototal replace	onerow										///
             format(%12.2fc) grplabels(1 Long @ 2 Short) 						///
             rowvarlabels savetex("${tex_tab}/Ecuador_FES_F2F_Length_Balance.tex")
			 
	iebaltab ${out_fes} if Modality == 2, grpvar(Form)  						///
			 nonote nototal replace	onerow										///
             format(%12.2fc) grplabels(1 Long @ 2 Short) 	 	 				///
             rowvarlabels savetex("${tex_tab}/Ecuador_FES_RM_Length_Balance.tex")
			 
	iebaltab ${out_lcs} if Modality == 1, grpvar(Form)   						///
			 nonote nototal replace	onerow										///
             format(%12.2fc) grplabels(1 Long(EN) @ 2 Short(FS)) 				///
             rowvarlabels savetex("${tex_tab}/Ecuador_LCS_F2F_Length_Balance.tex")
			 
	iebaltab ${out_cari}, grpvar(Modality) nonote onerow						///
			 nototal replace format(%12.2fc) grplabels(1 F2F @ 2 Remote)		///
             rowvarlabels savetex("${tex_tab}/Ecuador_CARI_Modality_Balance_Key.tex")
			  
	ren CARI_FES_Cat* 	C_FES_*
	ren rCARI_Inc_Cat*  rC_Inc_*
	
	asdoc ttest C_FES_1 == rC_Inc_1 if Modality == 1, unpaired replace save(${tabs}/Ecuador_F2F_FESINC_Unpaired_ttest.doc) ///
									   title(CARI vs. F2F rCARI) fhc(\b) fhr(\b) font(Open Sans) label dec(3) tzok
									   
	forval i = 2/4 {
	asdoc ttest C_FES_`i' == rC_Inc_`i' if Modality == 1, unpaired rowappend save(${tabs}/Ecuador_F2F_FESINC_Unpaired_ttest.doc) ///
				fhr(\b) font(Open Sans) label dec(3) tzok
	}
	
	asdoc ttest CARI_FES_Bad == rCARI_Inc_Bad if Modality == 1, unpaired rowappend save(${tabs}/Ecuador_F2F_FESINC_Unpaired_ttest.doc) ///
				fhr(\b) font(Open Sans) label dec(3) tzok
	...
	** disaggregation by groups 
	...
	iebaltab ${out_oth} if Modality == 1, grpvar(HH_Vul_FES)   						///
             ...
             rowvarlabels savetex("${tex_tab}/Ecuador_F2F_Outcoem_Balance_FS_Disaggregation.tex")
			 
	iebaltab ${out_oth} if Modality == 2, grpvar(HH_Vul_FES)   						///
			 nonote nototal replace	onerow										///
             format(%12.2fc) grplabels(0 Food Secure @ 1 Food Insecure) 				///
             rowvarlabels savetex("${tex_tab}/Ecuador_RM_Outcoem_Balance_FS_Disaggregation.tex")
