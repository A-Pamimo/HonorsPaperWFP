...
********************************************************************************
	...
	** REQUIRES:	${d_dta}/CAR_Full_Household_Analysis_`cdate'.dta
...
*******************************************************************************/
	...
	log using "${d_log}/cd04_hfc_sum_`cdate'.smcl", replace
	di `cdate'	

	use "${d_dta}/CAR_Full_Household_Analysis.dta", replace
	compress 
...
*******************************************************************************/
	...
	** 1 Survey Completion	
	...
	esttab using "${d_tex_tab}/CAR_rCARI_Completion.tex", 					///
		   cell("b(fmt(0)) rowpct(fmt(2))") unstack noobs modelwidth(15) 		///
		   collabels("Count" "Percent") nonumbers replace 

	/*
	dtable i.S_Assign_Modality i.S_Assign_Length i.S_Geo_Admin1 i.S_Geo_Admin2    ///
		   i.S_Geo_Admin3 i.S_Geo_Admin5 S_age i.S_relationship i.S_hh_status 	  ///
		   ...
		   i.S_floor_material_house, by(Completion, test) 						///
		   title(Myanmar rCARI Validation Study Progress)  					///
		   export(${d_tex_tab}/CAR_rCARI_Completion_by_Admin.docx, replace)     

		   */
		   
	** Survey Progress by Date
	tostring Sys_submission_time, g(Date_Str) format(%tdd_m_CY) force
	...
	catplot Modality, asyvars stack recast(bar) 			///
			...
			gap(1)) b1title("") 
	graph export "${d_tex_fig}/CAR_rCARI_Progress_Day.png", replace 
	...
	** F2F by Date
	catplot Modality if Modality == 1, asyvars stack recast(bar) 	///
			...
			gap(1)) b1title("") 		
	graph export "${d_tex_fig}/CAR_rCARI_Progress_Enum_F2F_Date.png", replace 
	...
	catplot Modality if Modality == 2, asyvars stack recast(bar) 	///
			...
			ytitle("") blabel(bar, pos(out) size(2)  		///
			gap(1)) b1title("") 		
	graph export "${d_tex_fig}/CAR_rCARI_Progress_Enum_RM_Date.png", replace 
...
/*	tostring today, g(Date_Str) format(%tdd_m_CY) force
	...
	graph export "${d_tex_fig}/CAR_rCARI_Duration_F2F_Date.png", replace
    ...
    ytitle("") legend(ring(4) pos(6) rows(5) size(3)) 
    graph export "${d_tex_fig}/CAR_rCARI_Duration_RM_Date.png", replace
*/    
	** 3 Household Size by Enumerator
    ...
    ytitle("") legend(ring(4) pos(6) rows(5) size(3)) 
    graph export "${d_tex_fig}/CAR_rCARI_HHSize_Modality.png", replace
...
*******************************************************************************/
	...
	* Randomization Balance
	iebaltab ${bal_all}, grpvar(Modality) nonote nototal replace		///
             ...
             rowvarlabels savetex("${d_tex_tab}/CAR_Demo_Modality_Balance.tex")
	
	iebaltab ${bal_f2f} if Modality == 1, grpvar(Form) nonote nototal replace	///
             format(%12.2fc) grplabels(1 F2F @ 2 Remote) 						///
             rowvarlabels savetex("${d_tex_tab}/CAR_Demo_F2F_Length_Balance.tex")
			 
	iebaltab ${bal_rm}  if Modality == 2, grpvar(Form) nonote nototal replace	///
             format(%12.2fc) grplabels(1 F2F @ 2 Remote)					///
             rowvarlabels savetex("${d_tex_tab}/CAR_Demo_RM_Length_Balance.tex")
	...
	* Outcome Balance by Survey Modality
	iebaltab ${outcome}, grpvar(Modality) nonote nototal replace		///
             ...
             rowvarlabels savetex("${d_tex_tab}/CAR_Outcome_Modality_Balance.tex")

	iebaltab ${outcome} if Modality == 1, grpvar(Form) nonote total replace	///
             format(%12.2fc) grplabels(1 Long @ 2 Short) onerow				///
             rowvarlabels savetex("${d_tex_tab}/CAR_Outcome_F2F_Length_Balance.tex")
			 
	iebaltab ${outcome} if Modality == 2, grpvar(Form) nonote total replace	///
             format(%12.2fc) grplabels(1 Long @ 2 Short) onerow				///
             rowvarlabels savetex("${d_tex_tab}/CAR_Outcome_RM_Length_Balance.tex")
